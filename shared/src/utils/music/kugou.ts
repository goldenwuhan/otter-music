...
export function getPlaylistCoverUrl(songs: KugouSongRaw[]): string {
  const cover = songs.find((s) => s.trans_param?.union_cover)?.trans_param?.union_cover;
...
export function buildKugouAndroidHeaders(url: string, dfid: string, mid: string): Record<string, string> {
...
export function buildKugouGlobalPlaylistSongsUrl(
  playlistId: string,
  page: number,
  dfid: string,
  mid: string,
...
export function buildKugouGlobalPlaylistInfoUrl(dfid: string, mid: string, body: string): string {
  const params: Record<string, string | number> = {
    appid: 1005,
    clientver: 20489,
    dfid,
...
export async function fetchKugouGlobalPlaylistPages(
  ...
  for (let page = 1; page <= 100; page += 1) {
    const url = buildKugouGlobalPlaylistSongsUrl(playlistId, page, dfid, mid);
  ...
  return fetchInfo ? withKugouGlobalPlaylistMeta(playlistId, detail, dfid, mid, fetchInfo) : detail;
}
...
export async function withKugouGlobalPlaylistMeta(
  ...
  try {
    ...
    const infoUrl = buildKugouGlobalPlaylistInfoUrl(dfid, mid, body);
    const response = parseKugouGlobalPlaylistInfoResponse((await fetchInfo(infoUrl, body)) || '');
...
export function buildKugouDeviceRegistrationPayload(mid: string): {
  url: string;
  ...
  const deviceParams = {
    ...
    imei: mid.substring(0, 15), imsi: '',
    manufacturer: 'Xiaomi', uuid: mid,
  ...
  const params: Record<string, string | number> = {
    appid: 1005, clientver: 20489, dfid: '-', mid, uuid: '-', clienttime: ct,
    part: 1, platid: 1, p,
  };
...
export function parseKugouDeviceRegistrationResponse(
  raw: Uint8Array,
  encryptKey: string,
  iv: string,
): { dfid: string } {
