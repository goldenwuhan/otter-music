...
import type { KugouPlaylistDetail } from '@otter-music/shared';
...
export { convertKugouSongToMusicTrack, isKugouGlobalCollectionId, KUGOU_PAGE_SIZE };
  ...
  if (IS_NATIVE) {
    ...
    return fetchKugouGlobalPlaylistPages(
      ...
      async (url, body) => {
        const res = await CapacitorHttp.request({
          method: 'POST', url,
          headers: { ...buildKugouAndroidHeaders(url, dfid, DEVICE_MID), 'Content-Type': 'application/json' },
